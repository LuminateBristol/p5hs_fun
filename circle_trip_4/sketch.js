x_dim = 1000;
y_dim = 1000;
circle_d = 1;
x_1_old=x_dim/2;
y_1_old=y_dim/2;
var slider_d;
n = x_dim/2.2;
m = 200;
e = 0;

function setup() {
  createCanvas(x_dim,y_dim);
  //slider_d = createSlider(1,100,1,10);
  //slider_d.position(10,10);
}

function draw() {
    background(0);
    //ellipse(x_dim/2,y_dim/2,n*2,n*2);
    //d = slider_d.value();

    e=e+0.1;

    for (let theta=0; theta<2*PI; theta=theta+(2*PI)/m) {
        x_1_old=x_dim/2;
        y_1_old=y_dim/2;
        a=1*sin(theta-PI/2.7)*400+500;
        b=3*cos(theta+PI/2.2)*80+350;
        c=2*sin(theta+PI/5)*180+500;
        for (let x_0 = 10; x_0<n; x_0=x_0+1) {
            y_0 = 10*(sin((2*PI*x_0)/a)+sin((2*PI*x_0)/b)+sin((2*PI*x_0)/c)+sin((2*PI*x_0)/c));
            x_1 = x_dim/2+x_0*cos(theta)+y_0*sin(theta);
            y_1 = y_dim/2-x_0*sin(theta)+y_0*cos(theta);
            stroke(0,0,255);
            line(x_1_old,y_1_old,x_1,y_1);
            x_1_old=x_1;
            y_1_old = y_1;
            //ellipse(x_1,y_1,circle_d,circle_d);
        }
    }

    for (let theta=0; theta<2*PI; theta=theta+(2*PI)/m) {
        x_1_old=x_dim/2;
        y_1_old=y_dim/2;
        a=1*sin(theta-PI/2.7)*400+500;
        b=3*cos(theta+PI/2.2)*80+350;
        c=2*sin(theta+PI/5)*180+500;
        for (let x_0 = 10; x_0<n; x_0=x_0+1) {
            y_0 = 10*(sin((2*PI*x_0)/a)+sin((2*PI*x_0)/b)+sin((2*PI*x_0)/c)+sin((2*PI*x_0)/c));
            x_1 = x_dim/2+x_0*cos(theta+radians(e))+y_0*sin(theta+radians(e));
            y_1 = y_dim/2-x_0*sin(theta+radians(e))+y_0*cos(theta+radians(e));
            stroke(255,0,0);
            line(x_1_old,y_1_old,x_1,y_1);
            x_1_old=x_1;
            y_1_old = y_1;
            //ellipse(x_1,y_1,circle_d,circle_d);
        }
    }
}